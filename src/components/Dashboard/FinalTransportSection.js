...
function FinalTransportSection(props) {
    ...
    const GetTransportData = () => {
        setLoading(true)
        getRequest({
            requesturl: getUrl() + `/CarStepSummary/GetCarSummaryForFinalTransport?userId=` + localStorage.getItem("UserId"),
        }).then((res) => {if (res) {               
    ...
    return (
                  ...
                  <h2 className="mb-20">Einde aflevering</h2>
                  {isLoading ? <React.Fragment>
          <DataLoader isDashboard={true}></DataLoader>
        </React.Fragment> :
            <>
                  <div className="d-grid grid-col4 col-gap-30 row-gap-26">
                    <ImageDiv imageSrc="images/Todo.Pending_icon.png" title="Nabellen" totalCount={TransportData.transportCallBackTotalCount} lateCount={TransportData.transportCallBackLateCount} todayCount={TransportData.transportCallBackTodayCount}></ImageDiv>
...
export default FinalTransportSection;